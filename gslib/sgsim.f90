...
!
    ...
    if(str(1:1) == ' ') str(1:20) = 'sgsim.par           '
    inquire(file=str,exist=testfl)
    if( .NOT. testfl) then
        write(*,*) 'ERROR - the parameter file does not exist,'
        write(*,*) '        check for the file and try again  '
        write(*,*)
        if(str(1:20) == 'sgsim.par           ') then
...
!
    ...
    if(MAXTMP > MAXDAT)MAXDAT = MAXTMP
!
...
!
    if(sill > (1.0+EPSLON) .OR. sill < (1.0-EPSLON)) then
...
!
    ...
    ! Trim this data?
    !
        if(var(icolvr) < tmin .OR. var(icolvr) >= tmax) then
            nt = nt + 1
            go to 3
        endif
        ntr = ntr + 1
    ...
    !
        if(icolvr > nvari .OR. icolwt > nvari) then
...
!
    ...
    if(testfl) then
        ...
        if(ixl > nvari .OR. iyl > nvari .OR. izl > nvari .OR.  &
        ivrl > nvari .OR. isecvr > nvari .OR. iwt > nvari) then
    ...
    !
        if(itrans == 1) then
            ...
            vrr,1.0)
            if(vrg < vrgtr(1)  ) vrg = vrgtr(1)
            if(vrg > vrgtr(ntr)) vrg = vrgtr(ntr)
            vr(nd) = vrg
        end if
        twt = twt + wt(nd)
...
!
    if(ktype >= 2) then
        ...
        do iz=1,nz
            do iy=1,ny
                ...
                !
                    if(trans .AND. ktype == 2 .AND. itrans == 1) then
                        if(vrr <= tmin .OR. vrr >= tmax) then
                            lvm(index) = -1.0e21
                        else   
                            call locate(vrtr,ntr,1,ntr,vrr,j)
                            j   =min(max(1,j),(ntr-1))
    ...
    !
        if(trans .AND. ktype == 4) then
            ...
            cp    = 0.0
            do i=1,nxyz
                if(lvm(i) > tmin .AND. lvm(i) <= tmax) then
...
!
    ...
    !
        do in=1,nxyz
            if((int(in/irepo)*irepo) == in) write(*,103) in
        ...
        !
            ...
            if(idbg >= 3) write(ldbg,141) p,sim(index)
            141 format(' random number ',f6.4,' realization ',f7.4)
        !
        ! Quick check for far out results:
        !
            if(abs(cmean) > 5.0 .OR. abs(cstdev) > 5.0 .OR.  &
            abs(sim(index)) > 6.0) then
    ...
    !
        ...
        do ind=1,nxyz
            simval = sim(ind)
            if(simval > -9.0 .AND. simval < 9.0) then
                ne = ne + 1
                av = av + simval
                ss = ss + simval*simval
            end if
            if(itrans == 1 .AND. simval > (UNEST+EPSLON)) then
...
!
    ...
    do 2 il=2,nlooku
        ...
        if(i < 1 .OR. j < 1 .OR. k < 1) go to 2
        if(i > nx .OR. j > ny .OR. k > nz) go to 2
...
!
    ...
    if(lktype >= 3) then
        ind = ix + (iy-1)*nx + (iz-1)*nxy
        if(lvm(ind) <= -6.0 .OR. lvm(ind) >= 6.0) then
            lktype = 0
            go to 33
        end if      
    end if
