...
!
    ...
    if(str(1:1) == ' ') str(1:20) = 'sisim.par           '
    inquire(file=str,exist=testfl)
    if( .NOT. testfl) then
        write(*,*) 'ERROR - the parameter file does not exist,'
        write(*,*) '        check for the file and try again  '
        write(*,*)
        if(str(1:20) == 'sisim.par           ') then
...
!
    ...
    if(testfl) then
        ...
        if(var(ivrl) < tmin .OR. var(ivrl) >= tmax)go to 55
        MAXDAT = MAXDAT + 1
        go to 55
        66 continue
        rewind(lin)
...
!
    ...
    if(testfl) then
        ...
        if(var(ivrl) < tmin .OR. var(ivrl) >= tmax)go to 77
        MAXTAB = MAXTAB + 1
        go to 77
        88 continue
        rewind(lin)
...
!
    if(nx > MAXX) stop 'nx is too big - modify .inc file'
    if(ny > MAXY) stop 'ny is too big - modify .inc file'
    if(nz > MAXZ) stop 'nz is too big - modify .inc file'
!
...
!
    ...
    !
        ...
        !
            ...
            if(imbsim == 0 .AND. tmp(index) /= 0.0) then
...
!
    ...
    !
        ...
        if(sim(index) > UNEST .OR. tmp(index) > 0.5) then
            if(sim(index) <= UNEST .AND. tmp(index) > 0.5) then
                ncsec  = ncsec + 1
                if(ncsec > maxsec) go to 1
            end if
            ncnode         = ncnode + 1
            icnode(ncnode) = il
