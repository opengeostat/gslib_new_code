! compile using the command: gfortran -static extractvar.f90 -o extractvar
...
!-----------------------------------------------------------------------
!
!   Extract variables from GSLIB file and save it in a new file
!
!-----------------------------------------------------------------------
    ...
    integer :: test, error, i,j
    real, allocatable :: table(:,:), newtable(:,:)
    ...
    read (*,'(a20)') str(1:20)
    if(str(1:1) == ' ') str(1:20) = 'extractvar.par        '
    ...
    read(lin,*,err=98) nvariables 
    write(*,*) 'Number of variables to extract: ', nvariables    
    
    read(lin,*,err=98) (varindex(i),i=1,nvariables)
    write(*,*) 'Column indexes:                 ', (varindex(i),i=1,nvariables)
    ...
    ! Finally extract variables
    allocate (newtable(maxdat,nvariables),stat = test)
    if (test /= 0) then
        write(*,*) 'Error 2: Allocation failed due to ','insufficient memory!', test
        stop
    end if
    call extractvar(table, maxdat, nvar, newtable, nvariables, varindex, error)
    if (error/=0) then 
...
subroutine extractvar(table, maxdat, nvar, newtable, nvariables, varindex, error)
